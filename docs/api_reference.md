...
| `newthread` | 发表新主题 | `fetchNewThreadForm()` / `submitNewThread()` | ✅ 已完成 | 📄 真实写入未验证 |
| `sendpm` | 发私信 | `fetchPmSendForm()` / `sendPrivateMessage()` | ✅ 已完成 | 📄 真实发送未验证 |
| `mypm` | 私信会话列表与详情 | `getPmList()` / `getPmConversation()` | ✅ 已完成 | ✅ 登录态只读实测 |
| `mynotelist` v3 | 帖子提醒 / 系统通知 | `getNoticeList()` | ✅ 已完成 | ✅ 两类登录态实测 |
| `friend` v1 | 好友列表 | `ForumToolsService.getFriendList()` | ✅ 已完成 | ✅ 模块探测 + fixture |
| `study_daily_attendance…` | 每日签到（Discuz 插件） | `ForumToolsService.dailySign()` | ✅ 已完成 | 📄 合成解析；手动触发后复核 |
| `showdarkroom&ajaxdata=json` | 小黑屋处罚列表 | `ForumToolsService.getDarkRoom()` | ✅ 已完成 | ✅ 公开只读实测 |
...
| `forum.php?mod=post&action=edit` | 编辑帖子 | `fetchEditPostForm()` / `submitEditPost()` | ✅ 已完成 | 📄 真实写入未验证 |
| `misc.php?mod=report` | 楼层举报（预取 HTML + POST） | `fetchReportForm()` / `submitReport()` | ✅ 已完成 | 📄 解析与 UI 测试 |
| 论坛附件 `swfupload` | 回复插图默认 → `[attachimg]aid[/attachimg]` | `ForumAttachmentUploadService` | ✅ 已完成 | 有 aid 时网页提交 + `attachnew` |
| 外链图床 `p.sda1.dev` | 备选插图 → `[img]url[/img]` | `ExternalImageUploadService` | ✅ 已完成 | |

---

## 楼层举报

举报沿用 Discuz Web 端点，不走 Mobile API：先 GET 预取表单隐藏字段，再 POST
`misc.php?mod=report&inajax=1`。客户端固定提供“广告垃圾、违规内容、恶意灌水、重复发帖、其他”五类原因，具体说明最多 200 字且必填；所有请求仍经 `S1HttpClient` 统一 Cookie、formhash、限速和 Web 代理处理。
...
```json
{
  ...
  "Variables": {
    ...
    "saltkey": "...",
    "member_uid": "10001",
    "member_username": "example_user",
    "member_avatar": "https://avatar.stage1st.com/avatar.php?uid=10001&size=small",
    "groupid": "53",
    "formhash": "abcdef12",
    "ismoderator": "0",
    "readaccess": "100",
...
> **客户端图片策略**：帖子正文以 `viewthread` → `postlist[].message` 内嵌 HTML 为准，**只信任服务端提供的 `<img src>`**（及可选的 `<a href>` 原图链接），不猜测 `.thumb.jpg` 或构造 `forum.php?mod=image` URL。inline 与全屏共用同一 URL；省流量靠 **下载门控**（始终 / 仅 Wi-Fi / 手动）、**每楼层 inline 图片张数上限**（默认 10，0 = 不限；超出显示「展开」且不挂载 `ImageViewer`）、**可配置磁盘缓存上限**（默认 256MB，可选 100/256/512）与 **客户端 `ResizeImage` 降采样**，磁盘仍缓存原图字节供全屏复用。头像可单独配置加载策略（始终 / 仅 Wi-Fi / 手动）；已磁盘缓存的头像不受门控阻塞。全屏看图不受正文图片门控限制。`attachmentImagePreviewList` 主要用于 **forumdisplay 列表封面**，详情页正文暂不依赖该字段。
...
- 预检：`GET api/mobile/index.php?module=newthread&version=4&fid={fid}`。
- 预检响应中的 `Variables.threadtypes` 只表示分类元数据；仍必须检查
  `Message.messageval` 和 `Variables.formhash`，不能把匿名返回分类误判为有发帖权限。
- 提交：`POST module=newthread&extra=&topicsubmit=yes&fid={fid}`，字段为
  `posttime`、`typeid`（服务端要求时必填）、`subject`、`message`、
  `allownoticeauthor=1`、`usesig=1`。客户端不发送服务端草稿 `save`。
- 成功以 `Message.messageval` 含 `succeed` 且 `Variables.tid` 非空为准；写入能力未使用真实账号进行线上验证。
...
| `rate` | string | `"0"` / `"1"` | **主题级**是否曾被评分（非楼层明细；`postlist` **不含**楼层 `rate`） |
...
> **评分（战斗力）**：Mobile JSON 的 `postlist` 项**不返回**楼层 `rate`；楼层评分明细只能从 HTML `viewthread&mobile=2` 的 `#ratelog_<pid>` 解析。`Variables.thread.rate` 仅表示主题曾被评分，不能用于按页/按楼跳过 HTML 拉取。
...
- `"login_succeed"` → 登录成功
- 其他 → 登录失败；优先展示已翻译的 `messagestr`（多子句中文提示补全句号），裸 key（如 `mobile:login_invalid`）由客户端映射为中文

---

## module=sendreply（发回复，当前实现）

> 回复提交：`POST api/mobile/index.php?module=sendreply&replysubmit=yes&version=4`。
...
1. 从回复/发帖/编辑页 HTML 解析 `hash` / `uid` / `fid`（及可选 `uploadUrl`）
2. `POST misc.php?mod=swfupload&action=swfupload&operation=upload&fid=` multipart（`Filedata` + `uid`/`hash`/…）
3. 响应为纯数字 `aid` 或 `DISCUZUPLOAD|0|{aid}|…`
4. `GET forum.php?mod=ajax&action=imagelist&type=single&aids=…&fid=…` 取缩略图
5. 正文写入 `[attachimg]{aid}[/attachimg]`；提交时附带 `attachnew[aid][description|readperm]`
6. 含论坛附件时回复/发新帖走**网页** `forum.php` 提交；无附件仍走 Mobile `sendreply` / `newthread`

**备选：外链图床**

上传至 `https://p.sda1.dev/api/v1/upload_external_noform?filename=`（原始字节，非 multipart），
将返回的 `data.url` 以 `[img]…[/img]` 写入 `message`。
**Web**：浏览器直连图床会 CORS 失败，经开发代理 `POST /ext-upload?filename=` 转发到同上游 URL（`scripts/proxy_server.dart`）。Native 仍直连。

上传前默认最长边压缩约 2000px（可开「原图」）；compose 内可切换「论坛 / 外链」。
...
#### 1. 上传附件（Discuz 原生，已实现）

请求：
- URL: `$baseUrl/misc.php`
- Method: POST
- Content-Type: `multipart/form-data`
- Query: `mod=swfupload&action=swfupload&operation=upload&fid={fid}`
- Cookie: 需要已登录态
- Referer: 对应回复/发帖/编辑页

multipart 字段：`uid`、`hash`、`type`、`id`、`size`、`filetype`、`Filedata`
...
| 字段 | 类型 | 示例值 | 说明 |
|------|------|--------|------|
| `uid` | string | `"10001"` | 用户 ID |
| `username` | string | `"example_user"` | 用户名 |
...
- 只读预检：`GET api/mobile/index.php?module=sendpm&version=4`，用于获取
  `formhash` 和登录/权限结果，不带 `pmsubmit`。
- 发送：`POST module=sendpm&pmsubmit=true`，字段为 `formhash`、`touid`、`message`。
- 成功状态：`messageval` 以 `_succeed`、`_success` 或
  `_succeed_mobile` 结尾，并要求 `Variables.pmid` 非空。
- POST 后超时、未知响应或成功但缺少 `pmid` 均视为“状态未知”，不得自动重发；
  客户端应先刷新会话再决定后续操作。
- 当前实现只做纯文本私信；真实账号发送未进行线上验证。

---

## module=friend v1（好友列表）

> `GET api/mobile/index.php?module=friend&version=1&uid={uid}`

响应 `Variables.list` 为好友数组（偶尔也可为 Map）；每项至少 `uid`、`username`。
头像由客户端按 UID 推导。未登录常见 `Message.messageval` 含
`login_before_enter_home`。当前 MVP 不猜分页。

代码：`ForumToolsService.getFriendList` → `/friends`。

---

## 每日签到（Discuz 插件）

> `GET /2b/study_daily_attendance-daily_attendance.html?inajax=1&formhash=...`
...
> `GET forum.php?mod=misc&action=showdarkroom&ajaxdata=json&cid={cursor}`

公开只读。响应：

- `message`：`{dataExist}|{nextCid}`，下一页 cursor 必须取这里的 cid
- `data`：以 UID 为 key 的对象，或数组（客户端两种都兼容）

每项字段：`cid`、`uid`、`username`、`action`、`reason`、`operator`、
`operatorid`、`dateline`、`groupexpiry`（可为「永不过期」）。空数据、空 cursor
或 cursor 不前进时结束加载。

代码：`ForumToolsService.getDarkRoom` → `/dark-room`。
...
分页使用 `count/perpage/page`。登录态只读实测已通过；解析契约另由合成 fixture 覆盖。
...
version=4 不可用。两类登录态实测已通过；合成 fixture 锁定字段契约。
...
| `author` | string | `"example_user"` | 作者用户名 |
| `authorid` | string | `"10001"` | 作者用户 ID |
...
| `sendpm` | 返回 JSON，未登录时提示 `to_login` | ✅ 可用（发私信，真实发送未验证） |
| `forum.php action=edit` | 返回 Web 编辑页；无权限时提示 `post_edit_nopermission` | ✅ 可用（编辑普通主题/回复，真实写入未验证） |
| `mypm` | 返回 JSON，`list` 字段；未登录 `login_before_enter_home` | ✅ 可用（私信列表） |
| `mynotelist` | version=3 返回 JSON；客户端按 mypost/system 使用，HTML 同分类兜底 | ✅ 已采用（两类登录态实测） |
