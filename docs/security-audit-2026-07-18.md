...
High / 相关 Medium 已在后续修复 PR 中落地，见 §9。
...
| ID | 状态 |
|:---|:---|
| H-1 Web 登出清代理会话 + formhash | 已修复 |
| H-2 img-proxy 收紧 + 禁用自动重定向 | 已修复 |
| H-3 外链 scheme 白名单 | 已修复 |
| M-1 脚本硬编码 Sentry DSN | 已修复（须轮换控制台 DSN） |
| M-2 Talker error 日志 | 已修复 |
| M-3 BBCode 属性消毒 | 已修复 |
| M-4 备份 ZIP 导入上限 | 已修复 |
| M-5 图床返回 URL 校验 | 已修复 |
| M-6 升级 URL 白名单 | 已修复 |
...
- M-7 Drift 明文 / 可选加密
- M-9 Web CSP
- M-10 强制代理 token（保留开发 DX；README 已加强警示）
- L-*（webview 依赖、CI audit、secure storage options 等）
- I-* 残余风险（无 pinning 等）
