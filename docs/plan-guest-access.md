...
S1 论坛 API（`forumindex` / `forumdisplay` / `viewthread`）本身支持无登录的游客读取。但当前 `home_screen.dart` 在未登录时对 **所有 Tab**（论坛/搜索/消息/我的）统一展示 `_LoginPrompt` 锁屏，完全阻断了游客浏览。

## 改造目标

游客可直接浏览论坛版块列表、帖子列表、帖子详情（只读），无需登录。写操作（回复/投票/发帖）仍需登录，触发时引导登录。
...
`checkAuthError` 检测 API 返回的 `to_login` 错误并抛出 `LoginRequiredException`。这个机制对游客是合理的：如果某个 API 端点确实需要登录，会展示 `S1ErrorView` 的"请先登录"界面并提供登录按钮。

### 6. `lib/widgets/s1_error_view.dart` — 无需改动

已正确处理 `LoginRequiredException`，展示锁图标 + "请先登录" + "去登录"按钮。
...
| `forum_list_provider.dart` | `getForumList()` API 支持游客访问 |
| `thread_list_provider.dart` | `getThreadList()` API 支持游客访问 |
| `compose_screen.dart` | 发帖/回复需登录，由路由层 FAB 控制入口 |
| `profile_screen.dart` | `ProfileBody` 内部已处理未登录状态 |

## 验收标准

1. **游客可浏览**：冷启动未登录 → 直接看到论坛版块列表，可点击进入版块查看帖子列表，可点击帖子查看详情。
