...
> 上次更新：2026-07-15（好友列表、手动每日签到、小黑屋、楼层举报、私信、编辑帖子）
...
| 服务端黑名单 | `home.php friend&view=blacklist` | 手动单向导入本地；不反向写入、不自动同步 | 已实现（只读导入） |
...
| 编辑帖子 | `forum.php action=edit` | `fetchEditPostForm()` / `submitEditPost()` + 编辑器 | 已实现；真实写入未验证 |
...
| 发私信 | `module=sendpm` | `fetchPmSendForm()` / `sendPrivateMessage()` + 会话内编辑器 | 已实现；真实发送未验证 |
...
- **私信详情**：对齐 `mypm&subop=view` 并应用内展示；登录态只读实测已通过；`sendpm` 已接入，真实发送未验证。
- **编辑帖子**：对齐 S1-Next 的 Web 编辑页 GET + `editsubmit=yes` POST；首版仅开放本人普通主题/回复，不处理特殊主题首帖及 Discuz 原生附件。
