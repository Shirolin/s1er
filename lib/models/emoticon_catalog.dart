/// S1 麻将脸目录。
///
/// 包定义真相源：`assets/emoticons/packs.json`（启动时加载）。
/// 下载清单：`assets/emoticons/download_list.txt`（维护脚本按此有目的拉取）。
/// 资源路径：`assets/emoticons/{dir}/{code}.{png|gif}`；后缀以 [applyManifest] 为准。
library;
...
class EmoticonPack {
  ...
  factory EmoticonPack.fromJson(Map<String, dynamic> json) {
    return EmoticonPack(
      title: json['title'] as String? ?? json['dir'] as String? ?? '',
      dir: json['dir'] as String,
      entityPrefix: json['prefix'] as String,
...
abstract final class EmoticonCatalog {
  ...
  static const packsAssetPath = 'assets/emoticons/packs.json';

  /// dataCode (`f:001`) → 相对路径 (`face2017/001.gif`)
  static final Map<String, String> _manifest = {};

  /// 与 `packs.json` 同步；未加载前使用内置回退（与当前入库一致）。
  static List<EmoticonPack> _packs = List<EmoticonPack>.of(_fallbackPacks);

  /// 表情包 Tab 顺序（麻将脸 → 动漫 → …）。
  static List<EmoticonPack> get packs =>
      List<EmoticonPack>.unmodifiable(_packs);

  /// 与 `assets/emoticons/packs.json` 当前内容一致，供测试 / 清单未加载时回退。
  static const _fallbackPacks = <EmoticonPack>[
    EmoticonPack(
      title: '麻将脸',
      dir: 'face2017',
      entityPrefix: 'f',
  ...
  /// 解析 `packs.json` 根对象或 `packs` 数组。
  static void applyPacksJson(Object? decoded) {
    final list = switch (decoded) {
      final List<dynamic> raw => raw,
      final Map<String, dynamic> map => map['packs'],
      _ => null,
    };
    if (list is! List || list.isEmpty) return;
    final parsed = <EmoticonPack>[];
    for (final entry in list) {
      if (entry is! Map) continue;
      final pack = EmoticonPack.fromJson(Map<String, dynamic>.from(entry));
      if (pack.dir.isEmpty || pack.entityPrefix.isEmpty || pack.count < 1) {
        continue;
      }
      parsed.add(pack);
    }
  ...
  static void resetPacksToFallback() {
    _packs = List<EmoticonPack>.of(_fallbackPacks);
  }
