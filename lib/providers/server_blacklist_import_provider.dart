...
class ServerBlacklistImportNotifier
  ...
  @override
  Future<BlacklistImportPreview?> build() async => null;
  ...
  Future<BlacklistImportPreview> loadPreview() async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      while (true) {
        final result =
            await service.getServerBlacklistPage(uid: uid, page: page);
    ...
    if (value == null) throw state.error ?? StateError('无法读取网页黑名单');
    return value;
  }
  ...
  Future<BlacklistImportResult> apply(BlacklistImportPreview preview) async {
    ...
    for (final user in preview.users) {
      ...
      final name = user.username.isNotEmpty ? user.username : existing.username;
